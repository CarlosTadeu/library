...
export class LibraryUserService {
    ...
    findAll(req?: any): Observable<EntityArrayResponseType> {
        ...
        return this.http.get<IBook[]>(`${this.resourceUrl}/${'all'}`, { params: options, observe: 'response' });
    }
    ...
    protected convertDateFromClient(libraryUser: ILibraryUser): ILibraryUser {
        const copy: ILibraryUser = Object.assign({}, libraryUser, {
            suspensionDate:
                libraryUser.suspensionDate && libraryUser.suspensionDate.isValid()
                    ? libraryUser.suspensionDate.format(DATE_FORMAT)
                    : undefined
        });
        return copy;
    }
